    ...
    public class AuthServiceRepository : IAuthService
    {
        ...
        public BaseResponseDto<string> Register(UserRegisterDto dto)
        {
            try
            {
                ...
                return new BaseResponseDto<string> { Success = true, Message = $"Token Expires At: {user.TokenExpiryDate}", Data = token };
            }
            catch (Exception ex)
            {
                return new BaseResponseDto<string> { Success = false, Message = $"An error occurred: {ex.Message}", Data = null };
            }
        }
